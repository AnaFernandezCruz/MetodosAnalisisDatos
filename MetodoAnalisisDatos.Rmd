...
```{r plot_missing_all, message= FALSE, warning=FALSE, fig.width=40, fig.height=30, echo=FALSE}
...
train %>%  dplyr::select(variablesTransformar) %>% plot_histogram()
...
```
 ...
 Y por último, trataremos de centrar y normalizar todas las variables numéricas una vez convertidas en factores en los anteriores pasos. Además quitaremos algunos outliers detectados en el apartado segundo de esta memoria en las variables más importantes. 
 La normalización se realizará pensando sobre todo en el paso de aplicación del método de selección de variables "lasso", que exige que todas las variables numéricas tengan varianza 1 y media 0 para evitar sesgos en el modelo resultante. El siguiente código realizará esta función:
...
listaPosOutliers <- c()
for(i in colnames(train_sin_na)) {
  if(is.numeric(train_sin_na[[i]]) && !is.factor(train_sin_na[[i]])  ) {
      if(i %in% c('LotArea','1stFlrSF','GrLivArea','TotalBsmtSF','SalePrice','Total_porch_SF','TotalSF'))   {
          outliersRosen <- rosnerTest(train_sin_na[[i]], k = 5, warn = F)
...
train_sin_na_outliers <- train_sin_na[-listaPosOutliers,] 
...
```
  ...
  Las columnas que a priori hemos quitado sin justificación, son las siguientes:
  * 'ExterCond','PoolArea','PoolQC','Exterior2nd','HeatingQC','Heating','Electrical': añaden inestabilidad al modelo al introducir autocorrelaciones en el mismo, sobre todo al crear las variables "dummies" en alguna de ellas.  Alguna de estas variables además tienen una varianza casi nula en el dataset de entrenamiento, por lo que nos vemos obligados a quitarlas.
  * 'MSSubClass' y 'Exterior1st' dan problemas a la hora de crear las variables "dummies" asociadas por los valores que estas columnas tienen. No hemos sido capaces de solventar el problema.
...
#trainTransformed <- train_sin_na_outliers
#testTransformed <- test_sin_na
#validationTransformed <- validation_sin_na
...
```
  ...
  El siguiente código comentado ayudó a eliminar las variables que estaban introduciendo autocorrelaciones en el modelo total:
...
varControl <-caret::trainControl(method="cv", number=5)
...
```
   ...
   Como vemos, lasso ha quitado una gran cantidad de regresores (curiosamente la cantidad varia en función de si lo ejecutamos como "chunk" o como "r-markdown"), demasiados para nuestro gusto, por lo que no podemos estar seguros de que haya funcionado correctamente.
...
valoresPredichosModeloLasso <- predict(lasso_mod,testTransformed,s=lasso_mod$bestTune$lambda)
...
Comprobamos con los anteriores gráficos los siguientes puntos:
  ...
  * La distribución de los resíduos es aproximadamente normal.
  * En la primera de las gráficas se observa que los residuos se distribuyen de forma más o menos homogénea excepto en los extremos en los que desaparece el patrón aleatorio. Esto parece indicar que no se han quitado correctamente todos los outliers posibles (pese a que se han quitado una gran cantidad). 
  * Existe un alto grado de heteroestaticidad (la varianza no es constante con las observaciones) que queda patente en el tercero de los gráficos y que hace que el modelo conseguido no sea fiable. No hemos sido capaces de eliminar esto de ninguna manera. El cuarto gráfico también muestra este hecho. 
...
```
  ...
  Hay que tener en cuenta que todas las variables numéricas han sido centradas y normalizadas para tener varianza uno. La siguiente función deshace el cambio de variable para poder comparar los precios originales con los precios transformados:
...
#cambioVariableInversoTest <- revPredict(preProcValuesTest, testTransformed)
#exp(cambioVariableInversoTest$SalePrice)

data.frame(cbind(preciosOriginales=testTransformed$SalePrice,predicted=valoresPredichosModeloLasso))
