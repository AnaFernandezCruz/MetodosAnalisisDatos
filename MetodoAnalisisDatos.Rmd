...
```
  ...
  Una vez realiazado el proceso de separación del dataset completo en tres partes (test, train y validación final) realizaremos un primer resumen del estdo del dataset:
...
for (i in columnas_sustitucion_nas) {
  full_dataset[i][is.na(full_dataset[i])] <- "NA"
}
...
```
  ...
  A la vista de estos diagramas, observamos lo siguiente:
    ...
    El siguiente código almacenará estas variables para su posterior procesado en otras fases de la práctica:

```{r}
library(caret)
elementosBajaVarianza <- nearZeroVar(train_test,freqCut = 95/5,uniqueCut = 10)
colnames(train_test)[elementosBajaVarianza]
```
...
```{r}
#train_dataset %>%  dplyr::select(var_ordinales)  %>% plot_bar()
...
```
 ...
 En cuanto a la detección de Outliers
...
```{r warning=FALSE}
    ...
    (na_counts_sort)
    train_test_na <- train_test %>% dplyr::select(c('LotFrontage','GarageYrBlt','MasVnrType','MasVnrArea','MSZoning', 'Functional','Exterior1st','Exterior2nd','Electrical','BsmtFinSF1','BsmtFinSF2','BsmtUnfSF','KitchenQual', 'TotalBsmtSF','BsmtFullBath','BsmtHalfBath','GarageCars','GarageArea','SaleType'))
...
test<- test %>% dplyr::select( -LotFrontage)
train_test<- train_test %>% dplyr::select(-LotFrontage)
train_test_na<- train_test_na %>% dplyr::select(-LotFrontage)
full_dataset<- full_dataset %>% dplyr::select( -LotFrontage)
```
...
```{r}
library(tidyverse)
vecindario_precioGaraje<- train_test %>% dplyr::select(Neighborhood, GarageYrBlt)
...
var_nominales <- c('MSSubClass','MSZoning','Street','Alley','LandContour','LotConfig','Neighborhood','Condition1','Condition2','BldgType','HouseStyle','RoofStyle','RoofMatl','Exterior1st','Exterior2nd','MasVnrType','Foundation','Heating','GarageType','MiscFeature', 'SaleType','SaleCondition')     
