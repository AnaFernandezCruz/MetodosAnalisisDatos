...
preProcValues <- preProcess(train_sin_na  %>% dplyr::select(-OverallQual, -OverallCond),method = c("center", "scale"))
...
trainTransformed <- predict(preProcValues, train_sin_na)
testTransformed <- predict(preProcValues, test_sin_na)
dummies <- dummyVars(reformulate(termlabels = var_modelo_with_SalePrice ),fullRank = T, data = data.frame(trainTransformed),sep=NULL)
trainTransformedDummies <- predict(dummies, newdata = trainTransformed)
testTransformedDummies <- predict(dummies, newdata = testTransformed)
