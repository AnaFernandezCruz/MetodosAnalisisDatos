...
Life_Expectancy_Data_PaisSubregion <- merge(x=Life_Expectancy_Data,y=codigosPais,by.x =1 , by.y = 1,all.x=TRUE) %>% dplyr::select(Pais,sub.region) %>% dplyr::distinct()
...
Datos_estudio_PaisSubregion <- merge(x=Datos_estudio,y=codigosPais,by.x =1 , by.y = 1,all.x=TRUE) %>% dplyr::select(Pais,sub.region) %>% dplyr::distinct()
...
if(!file.exists(paste(pathDatos,"Life_Expectancy_Data_Corregidos.csv",sep=""))) {
    ...
    Datos_estudio <- subset(Datos_estudio, select =-c(fertility,ppgdp,lifeExpF,pctUrban))
    ...
    Life_Expectancy_Data<- subset(Life_Expectancy_Data, select =-c(fertility,ppgdp,lifeExpF,pctUrban))
    Life_Expectancy_DataSinPaisNiAño = subset(Life_Expectancy_Data, select =-c(Pais,Año))
    
    rulesCorrectionRanges <-correctionRules( paste( pathDatos , "rulesAsignacionNAs.txt" , sep=""))
    corrections3 <- correctWithRules(rulesCorrectionRanges,Datos_estudio)
    Datos_estudio <- corrections3$corrected
  
    corrections4 <- correctWithRules(rulesCorrectionRanges,Life_Expectancy_Data)
    Life_Expectancy_Data <- corrections4$corrected
    
    write.csv(Datos_estudio, "datos_practica_Corregidos.csv", row.names=FALSE)
    write.csv(Life_Expectancy_Data, "Life_Expectancy_Data_Corregidos.csv", row.names=FALSE)
   
    
}else{
  Datos_estudio = read.csv(paste(pathDatos,"datos_practica_Corregidos.csv",sep=""),na.strings=c("NA","NaN", ""))
  Datos_estudioSinPaisNiAño = subset(Datos_estudio, select = -c(Pais,Año) )
  Life_Expectancy_Data = read.csv(paste(pathDatos,"Life_Expectancy_Data_Corregidos.csv",sep=""),na.strings=c("NA","NaN", ""))
  Life_Expectancy_DataSinPaisNiAño = subset(Life_Expectancy_Data, select =-c(Pais,Año))
}
